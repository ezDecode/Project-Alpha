...
/**
 * A memoized component for rendering individual letters of the heading.
 * It uses a spring animation to change font weight based on mouse proximity.
 */
 ...
 * Displays the main heading, subheading, and Navbar.
 */
...
const HeroSection: React.FC<HeroSectionProps> = ({ startAnimation, isScrolled }) => {
  ...
  // These transforms create the parallax scroll effect for the content
  const opacity = useTransform(scrollYProgress, [0.5, 1], [1, 0]);
  const scale = useTransform(scrollYProgress, [0.5, 1], [1, 0.9]);

  // Effect to track mouse position for the heading animation
  useEffect(() => {
  ...
  const imageInsertionIndex = useMemo(() => Math.floor(Math.random() * (wordsPart1.length - 1)), [wordsPart1.length]);
  ...
  const childVariants: Variants = { 
    ...
    visible: { filter: 'blur(0px)', opacity: 1, y: 0, transition: { duration: 0.6, ease: [0.25, 0.25, 0, 1] as const } } 
  };
  ...
  return (
    <div ref={targetRef} id="home" className="panel relative h-screen w-screen overflow-hidden" style={{ backgroundColor: '#FAF6E9' }}>
      ...
      {/* Navbar receives the isScrolled prop to manage its text color */}
      ...
      <motion.div style={{ opacity, scale }} className="flex flex-col items-center justify-center h-full w-full relative z-10">
        <div className="text-center w-[70vw] mx-auto relative">
          <motion.h1 
            ref={headingRef} 
            className="text-black text-[3.85rem] md:text-[6rem] lg:text-[8.25rem] font-light text-center mb-4 mt-[12vh] cursor-default flex flex-wrap justify-center items-center" 
            style={{ fontFamily: "'PP Editorial New', serif", fontWeight: 300, lineHeight: '1.2', letterSpacing: '-0.03em' }} 
            variants={containerVariants} 
            initial="hidden" 
            animate={startAnimation ? 'visible' : 'hidden'}
          >
            {/* Renders the first part of the heading */}
            {wordsPart1.map((word, index) => (
              <React.Fragment key={`p1-${index}`}>
                ...
                {/* Inserts the animated image component at a random position */}
                {index === imageInsertionIndex && (
                  <>
                    <motion.div
                      className="relative lg:-translate-y-3 md:-translate-y-3 -translate-y-2 w-24 h-20 md:w-32 md:h-28 lg:w-48 lg:h-32 bg-neutral-800 rounded-full inline-flex overflow-hidden align-middle mx-2"
            ...
            {/* Renders the second part of the heading */}
          ...
          <TextAnimate 
            ...
            delay={0.8} 
            duration={0.3} 
            className="text-center font-polysans text-[2rem] md:text-[2.15rem] font-tight" 
            style={{ fontWeight: 400, color: 'black', lineHeight: '1.5' }} 
      ...
      {/* SVG curve for the transition to the next section */}
