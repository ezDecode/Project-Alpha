...
const Letter = memo<LetterProps>(({ char, isItalic, mousePos, variants }) => {
  ...
  const weightSpring = useSpring(weightMotionValue, {
    damping: 12,
    stiffness: 250,
    mass: 0.8,
  });
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const distance = Math.sqrt(
        Math.pow(centerX - mousePos.current.x, 2) + 
        Math.pow(centerY - mousePos.current.y, 2)
      );
...
const HeroSection: React.FC = () => {
  ...
  const images = [
    ...
    "/assets/HeaderImageBW.png",
    "/assets/HeaderImageRed.png",
    // Add other images to this array if you have them, otherwise it will cycle between the two
  ];
    ...
    <div ref={targetRef} id="home" className="panel relative h-screen w-screen overflow-hidden bg-black">
      ...
      <motion.div style={{ opacity, scale }} className="flex flex-col items-center justify-center h-full w-full relative z-10">
        <div className="text-center w-[70vw] mx-auto relative">
          ...
          <TextAnimate by="word" delay={0.5} duration={0.5} className="text-center font-polysans text-[2rem] md:text-[2.05rem]" style={{ fontWeight: 400, color: '#d4d4d4', lineHeight: '1.5' }}>
